...
class DiscussControllerLock(DiscussController):
    ...
    def mail_attachment_delete(self, attachment_id, access_token=None, **kwargs):
        ...
        if attachment_sudo and \
        (
            attachment_sudo.res_model == 'mail.channel' \
            or isinstance(request.env[attachment_sudo.res_model], request.env['mail.thread'].__class__)
            ):
